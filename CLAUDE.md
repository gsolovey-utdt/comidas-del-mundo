...
| `images/foods/` | Imágenes de las comidas (JPG 768×768, recortadas a 16:11 por CSS) |
...
| 2026-05 | Imágenes con `object-fit: cover` | Decisión Guillermo: la imagen siempre llena la caja sin bandas blancas. En la práctica las 51 imágenes son **768×768** (cuadradas, no 16:11), así que el recorte top/bottom centrado lo hace siempre la caja `.food-image-wrap` (`aspect-ratio: 16/11`). La regla real para futuras comidas: mantener 768×768 para ser consistente con el resto. Si el sujeto no queda centrado y el recorte corta algo importante, ajustar `object-position` puntualmente. |
...
| 2026-06-16 | Corregida documentación: imágenes son 768×768, no 960×660/16:11 | Al agregar `cachupa.jpg`/`locos.jpg` se asumió (por esta misma doc) que el resto cumplía 16:11. Guillermo verificó y las 51 imágenes existentes son cuadradas 768×768; la "regla 960×660" nunca se cumplió en la práctica. El recorte a 16:11 vía `object-fit: cover` siempre fue automático y universal, no una excepción a vigilar por imagen. |
...
- **Revisión de imágenes de comidas**: auditar todo el set en `images/foods/` por (1) que el sujeto quede centrado dentro del cuadrado 768×768, ya que `.food-image-wrap` siempre recorta arriba/abajo a 16:11 vía `object-fit: cover` — y (2) calidad visual general (nitidez, encuadre, iluminación). Ajustar `object-position` puntualmente donde el recorte corte algo importante.
- **¿Adaptar `.food-image-wrap` al aspect ratio real de las imágenes?**: las 51 imágenes son 768×768 (cuadradas) pero la caja fuerza `aspect-ratio: 16/11` con `object-fit: cover`, así que todas se recortan arriba/abajo para encajar — viene pasando desde siempre, no es un bug nuevo (detectado 2026-06-16). Dos caminos posibles, sin decidir todavía: (a) cambiar la caja a un aspect más cuadrado, o (b) regenerar/re-encuadrar las 51 imágenes a 16:11. Trade-offs: (a) implica re-tunear las medidas fijas por pantalla (caja 900×680 desktop, breakpoints mobile) ya ajustadas para que las 6 pantallas entren sin scroll; (b) no resuelve nada en las imágenes donde el sujeto ya toca los bordes del cuadrado (ej. `locos.jpg`), porque recortar offline a 16:11 es el mismo recorte que ya hace el CSS hoy — sólo ayudaría si se consiguen fuentes nuevas ya encuadradas en 16:11 desde el origen.
...
- Las imágenes en `images/foods/` están normalizadas a **768×768** (cuadradas, no 16:11 — verificado sobre las 51 imágenes actuales el 2026-06-16). La CSS de `#food-image` usa `object-fit: cover` dentro de `.food-image-wrap` (`aspect-ratio: 16/11`), así que **todas** las imágenes se recortan arriba/abajo (recorte centrado por default) para llenar la caja sin bandas blancas. Al agregar una nueva, mantener 768×768 para ser consistente con el resto. Si el sujeto no queda centrado y el recorte corta algo importante, ajustar `object-position` puntualmente para esa imagen.
